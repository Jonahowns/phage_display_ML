...
if __name__ == "__main__":
    ...
    requiredNamed.add_argument("-k", "--hidden_param_key", type=str, help="Which hyperparameter dictionary from hyp_configs to use", required=True)
    requiredNamed.add_argument("-t", "--trials", type=str, help="How many trials for optuna to use", required=True)
    requiredNamed.add_argument("-e", "--epochs", type=str, help="How many epochs should trials be run", required=True)
    requiredNamed.add_argument("-g", "--gpus", type=str, help="How many gpus total", required=True)
    # optional arguments
    ...
    o = open(f'./submission_templates/optuna_template.sh', 'r')
    filedata = o.read()
    o.close()
    ...
    with open(os.path.join(datatype_folder, f"{args.hidden_param_key}_{run_data['model_type']}_{out}.sh"), 'w+') as file:
        file.write(filedata)
