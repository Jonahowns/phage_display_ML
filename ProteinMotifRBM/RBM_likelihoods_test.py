...
def all_weights(RBMin, name, rows, columns, h, w, rbm_loaded=False, molecule='rna'):
    if rbm_loaded:
        ...
        fig = sequence_logo.Sequence_logo_all(RBMin.weights[order], name=name + '.pdf', nrows=rows, ncols=columns, figsize=(h,w) ,ticks_every=10,ticks_labels_size=10,title_size=12, dpi=400)
    else:
        RBM = Proteins_RBM_utils.loadRBM(RBMin)
        beta = Proteins_RBM_utils.get_beta(RBM.weights)
        order = np.argsort(beta)[::-1]
        fig = sequence_logo.Sequence_logo_all(RBM.weights[order], name=name + '.pdf', nrows=rows, ncols=columns, figsize=(h,w) ,ticks_every=10,ticks_labels_size=10,title_size=12, dpi=400)


def get_likelihoods(rbm, datafile, outfile, molecule="protein"):
    data = Proteins_utils.load_FASTA(datafile,drop_duplicates=True, type=molecule)
...
# RBM = Proteins_RBM_utils.loadRBM('./trial/trbm1')

rpath = "../../pytorch-rbm/"

RBM = Proteins_RBM_utils.loadRBM('./long_trial')

all_weights(RBM, './long_trial_weights', 5, 1, 10, 2, rbm_loaded=True, molecule='protein')
get_likelihoods(RBM, pytorch_test, './long_trial_likeli.txt')
