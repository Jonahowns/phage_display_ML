...
def load_FASTA(filename,with_labels=False, remove_insertions = True,drop_duplicates=True, type='protein'):
    count = 0
    current_seq = ''
    all_seqs = []
    if with_labels:
    ...
    with open(filename,'r') as f:
        ...
        if type == 'protein':
            ...
            all_seqs = np.array(map(lambda x: [rnadict[y] for y in x], all_seqs[1:]), dtype=int, order="c")
        elif type == 'dna':
            all_seqs = np.array(map(lambda x: [dnadict[y] for y in x], all_seqs[1:]), dtype=int, order="c")
