...
class pcrbm_cluster(Base_drelu):
    ...
    def training_step_PCD_free_energy_gmm(self, batch, batch_idx):
        ## differences in clustering alg
        ## 1) Remove reshuffle steps, only have clustering which serves as both
    ...
    def delete_cluster(self, indx):
        for key in self.hidden_convolution_keys[indx]:
            for param in ["theta", "gamma", "0theta", "0gamma"]:
                for mod in ["+", "-"]:
                    delattr(self, f"{key}_{param}{mod}")
            delattr(self, f"{key}_W")
    ...
    def replace_cluster(self, source_indx, dest_indx):
        ...
        dest_keys = self.hidden_convolution_keys[dest_indx]
        with torch.no_grad():
    ...
    def cluster_gmm(self):
        ...
        clust_num = best_gmm.n_components
        gmm_cluster_assignments = best_gmm.predict(X)
        ...
        gmm_cluster_assignments = torch.tensor(gmm_cluster_assignments, device=self.device)

        # figure out cluster mapping
        nclust_totals = [(gmm_cluster_assignments == i).int().sum().item() for i in range(clust_num)]
        try:
            preferred_cluster = [round(current_clusters[gmm_cluster_assignments == i].mode()[0].item()) for i in
                                range(clust_num)]
        except IndexError:
            print('hiya')
        preferred_fraction = [(i, (
                    current_clusters[(current_clusters[gmm_cluster_assignments == i])] == preferred_cluster[
                i]).sum().item() / nclust_totals[i])
                              for i in range(clust_num)]
        ...
        # create initial mapping from gmm assignments to crbm cluster assignments
        gmm_source, crbm_dest, gmm_remaining = [], [], []
        for clust, frac in preferred_fraction:
            clust_choice = preferred_cluster[clust]
            if clust_choice in crbm_dest:
                gmm_remaining.append(clust)
            else:
                gmm_source.append(clust)
                crbm_dest.append(clust_choice)

        crbm_clusters = [i for i in range(self.clusters.item())]
        remaining_crbm_clusters = sorted(list(set(crbm_clusters) - set(crbm_dest)))

        for hid, hr in enumerate(gmm_remaining):
            gmm_source.append(hr)
            crbm_dest.append(remaining_crbm_clusters[hid])

        # too few gmm clusters, replace and delete crbm clusters
        if clust_diff > 0:
            # Get remaining clusters again
            remaining_crbm_clusters = sorted(list(set(crbm_clusters) - set(crbm_dest)))
        ...
        ### Finally we will rewrite our cluster assignments
        masks = [gmm_cluster_assignments == i for i in range(clust_num)]
        for i in range(clust_num):
            h_source = gmm_source[i]
            c_dest = crbm_dest[i]
            gmm_cluster_assignments[masks[h_source]] = c_dest

        self.cluster_assignments = gmm_cluster_assignments

        self.update_clust_totals()
        print("#PostGMMCluster", file=o)
        self.report_cluster_membership(o)
        o.close()
    ...
    def pearson_loss(self, Ih):
        ...
        interactions = torch.zeros((1,), device=self.device)
        interactions.add_(pearson_mat.triu(diagonal=1).sum())
        interactions.sub_(self_interaction_term)
