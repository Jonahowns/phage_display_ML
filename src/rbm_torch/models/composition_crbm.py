...
from rbm_torch.utils.utils import Categorical, fasta_read, conv2d_dim, conv1d_dim, pool1d_dim, BatchNorm1D  #Sequence_logo, gen_data_lowT, gen_data_zeroT, all_weights, Sequence_logo_all,



class comp_CRBM(LightningModule):
    def __init__(self, config, debug=False, precision="double", meminfo=False):
        ...
        total_intermediate_hidden_units = 0
        for key in self.hidden_convolution_keys:
            ...
            total_intermediate_hidden_units += self.convolution_topology[key]["number"]
        ...
        self.h_num = config["h_num"]

        self.final_conv_topology = {"kernel": total_intermediate_hidden_units, "number": self.h_num, "stride":1, "dilation":1, "padding":0, "output_padding":0}

        final_h_dim = conv1d_dim([self.batch_size, 1, total_intermediate_hidden_units], self.final_conv_topology)
        self.final_conv_topology["weight_dims"] = final_h_dim["weight_shape"]

        self.register_parameter(f"final_W", nn.Parameter(self.weight_intial_amplitude * torch.randn(self.final_conv_topology["weight_dims"], device=self.device)))
    ...
    def energy_PT(self, v, h, N_PT, remove_init=False):
        ...
        for i in range(N_PT):
            E[i] = self.energy_v(v[i], remove_init=remove_init) + self.energy_h(h, remove_init=remove_init) - self.bidirectional_weight_term(v[i], h)
        return E
    ...
    def energy_h(self, config, remove_init=False):
        ...
        # for iid, i in enumerate(self.hidden_convolution_keys):
        if remove_init:
            a_plus = getattr(self, f'gamma+').sub(getattr(self, f'0gamma+')).unsqueeze(0)
            a_minus = getattr(self, f'gamma-').sub(getattr(self, f'0gamma-')).unsqueeze(0)
            theta_plus = getattr(self, f'theta+').sub(getattr(self, f'0theta+')).unsqueeze(0)
            theta_minus = getattr(self, f'theta-').sub(getattr(self, f'0theta-')).unsqueeze(0)
        else:
            a_plus = getattr(self, f'gamma+').unsqueeze(0)
            a_minus = getattr(self, f'gamma-').unsqueeze(0)
            theta_plus = getattr(self, f'theta+').unsqueeze(0)
            theta_minus = getattr(self, f'theta-').unsqueeze(0)
        ...
        config_minus = -1*torch.maximum(-config, zero)

        return ((config_plus.square() * a_plus) / 2 + (config_minus.square() * a_minus) / 2 + (config_plus * theta_plus) + (config_minus * theta_minus)).sum(1)
    ...
    def random_init_config_h(self, zeros=False, custom_size=False):
        ...
        if zeros:
            config = torch.zeros(size, device=self.device)
        else:
            config = torch.randn(size, device=self.device)
    ...
    def logpartition_h(self, inputs, beta=1):
        ...
        if beta == 1:
            ...
            theta_plus = (beta * getattr(self, f'theta+') + (1 - beta) * getattr(self, f'0theta+')).unsqueeze(0)
            theta_minus = (beta * getattr(self, f'theta-') + (1 - beta) * getattr(self, f'0theta-')).unsqueeze(0)
            a_plus = (beta * getattr(self, f'gamma+') + (1 - beta) * getattr(self, f'0gamma+')).unsqueeze(0)
            a_minus = (beta * getattr(self, f'gamma-') + (1 - beta) * getattr(self, f'0gamma-')).unsqueeze(0)

            # in_neg = inputs[iid][:, :, 1]
            # in_pos = inputs[iid][:, :, 0]
        y = torch.logaddexp(self.log_erf_times_gauss((-inputs + theta_plus) / torch.sqrt(a_plus)) -
                            0.5 * torch.log(a_plus), self.log_erf_times_gauss((inputs + theta_minus) / torch.sqrt(a_minus)) - 0.5 * torch.log(a_minus)).sum(
            1) + 0.5 * np.log(2 * np.pi) * inputs.shape[1]
    ...
    def compute_output_v(self, X):  # X is the one hot vector
        ...
        output = F.conv1d(flat_out.unsqueeze(1).type(torch.get_default_dtype()), getattr(self, "final_W"), stride=self.final_conv_topology["stride"],
                 padding=self.final_conv_topology["padding"],
                 dilation=self.final_conv_topology["dilation"]).squeeze(2)
    ...
    def compute_output_h(self, Y):  # from h_uk (B, hidden_num)
        ...
        dconv = F.conv_transpose1d(Y.unsqueeze(2), getattr(self, f"final_W"),
                                   stride=self.final_conv_topology["stride"],
                                   padding=self.final_conv_topology["padding"],
                                   dilation=self.final_conv_topology["dilation"],
                                   output_padding=self.final_conv_topology["output_padding"]).squeeze(1)

        outputs = []
        nonzero_masks = []
        current_idx = 0
        for iid, i in enumerate(self.hidden_convolution_keys):
            number = self.convolution_topology[i]["number"]

            reconst = self.unpools[iid](dconv[:, current_idx: current_idx+number].unsqueeze(2), self.max_inds[iid])
            ...
            nonzero_masks.append((outputs[-1] != 0.).type(torch.get_default_dtype()))
            # * getattr(self, "hidden_layer_W")[iid])  # Used for calculating mean of outputs, don't want zeros to influence mean
    ...
    def on_before_zero_grad(self, optimizer):
        with torch.no_grad():
            ...
            getattr(self, "final_W").data.clamp_(0.0, 1.0)
    ...
    def training_step_CD_free_energy(self, batch, batch_idx):
        ...
        final_W_shape = self.final_conv_topology["weight_dims"]
        x = torch.sum(torch.abs(getattr(self, f"final_W")), (2, 1)).square()
        reg2 += x.mean() * self.l1_2 / (2 * final_W_shape[1] * final_W_shape[2])
    ...
    def cgf_from_inputs_h(self, I):
        with torch.no_grad():
            ...
            sqrt_gamma_plus = torch.sqrt(getattr(self, f"gamma+")).expand(B, -1)
            sqrt_gamma_minus = torch.sqrt(getattr(self, f"gamma-")).expand(B, -1)
            log_gamma_plus = torch.log(getattr(self, f"gamma+")).expand(B, -1)
            log_gamma_minus = torch.log(getattr(self, f"gamma-")).expand(B, -1)

            Z_plus = -self.log_erf_times_gauss((-I + getattr(self, f'theta+').expand(B, -1)) / sqrt_gamma_plus) - 0.5 * log_gamma_plus
            Z_minus = self.log_erf_times_gauss((I + getattr(self, f'theta-').expand(B, -1)) / sqrt_gamma_minus) - 0.5 * log_gamma_minus
            map = Z_plus > Z_minus
