...
class pool_class_CRBM(pool_CRBM):
    def __init__(self, config, debug=False, precision="double", meminfo=False):
        ...
        assert self.objective in ["classification", "regression"]

        self.alpha = config["alpha"]

        self.classes = config["classes"]
        self.register_parameter(f"y_bias", nn.Parameter(torch.zeros(self.classes, device=self.device)))
        self.register_parameter(f"y_bias0", nn.Parameter(torch.zeros(self.classes, device=self.device)))
        for key in self.hidden_convolution_keys:
            self.register_parameter(f"M_{key}", nn.Parameter(0.05 * torch.randn((self.convolution_topology[key]["number"], self.classes), device=self.device)))
    ...
    def free_energy_discriminative(self, v, y):
        ...
        numerator = torch.index_select(getattr(self, "y_bias"), 0, y) + self.logpartition_h(h_input)

        h_input_all_classes = self.compute_output_y_for_h(y, all_classes=True)
        denom = torch.zeros((self.classes, *numerator.shape), device=self.device)
        for c in range(self.classes):
            h_in = [torch.add(h_input_v[i], h_input_all_classes[i][:, c]) for i in range(len(h_input_v))]
            denom[c] = torch.index_select(getattr(self, "y_bias"), 0, torch.tensor(c, device=self.device).expand(h_in[0].shape[0])) + self.logpartition_h(h_in)

        full_denom = torch.logsumexp(denom, 0)

        return -(numerator - full_denom)
    ...
    def energy(self, v, h, y, remove_init=False, hidden_sub_index=-1):
        return self.energy_v(v, remove_init=remove_init) + self.energy_h(h, sub_index=hidden_sub_index, remove_init=remove_init) - \
               self.bidirectional_weight_term(v, h, hidden_sub_index=hidden_sub_index) + self.energy_y(y, remove_init=remove_init) + self.classification_weight_term(h, y)
    ...
    def classification_weight_term(self, h, y, hidden_sub_index=-1):
        ...
        return torch.index_select(hm, 1, y)
    ...
    def energy_y(self, config, remove_init=False):
        if remove_init:
            ybias = getattr(self, "y_bias") - getattr(self, "y_bias0")
        else:
    ...
    def logpartition_y(self, inputs, beta=1):
        if beta == 1:
            ...
            return torch.logsumexp((beta * getattr(self, "y_bias") + (1 - beta) * getattr(self, "y_bias0")) + beta * inputs, 1)
    ...
    def compute_output_h_for_y(self, H):
        # h is list of shapes shape (batch_size, h_num)
        outputs = []
        for kid, key in enumerate(self.hidden_convolution_keys):
            outputs.append(torch.mm(H[kid], getattr(self, f"M_{key}")))
    ...
    def compute_output_y_for_h(self, Y, all_classes=False):
        ...
        for key in self.hidden_convolution_keys:
            if all_classes:
                outputs.append(torch.swapaxes(getattr(self, f"M_{key}"), 1, 0).expand(Y.shape[0], -1, -1))
            else:
                outputs.append(torch.index_select(torch.swapaxes(getattr(self, f"M_{key}"), 1, 0), 0, Y))
        return outputs
    ...
    def sample_from_inputs_y(self, psi):
        ...
        probs = F.log_softmax(psi + getattr(self, "y_bias").unsqueeze(0), dim=1)
    ...
    def training_step_CD_free_energy(self, batch, batch_idx):
        ...
        D_v = self.free_energy_discriminative(V_pos_oh, y_pos).mean()
        D_vp = self.free_energy_discriminative(V_neg_oh, y_neg).mean()
        discriminative_cd_loss = D_v - D_vp

        hybrid_loss_function = (1 + self.alpha) * discriminative_cd_loss + self.alpha * unsupervised_cd_loss
        ...
        # Calculate Loss
        if self.use_test_regularization:
            loss = hybrid_loss_function + reg1 + reg2 + reg3 + bs_loss + gap_loss + div_loss
        else:
            loss = hybrid_loss_function + reg1 + reg2 + reg3 + gap_loss + bs_loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        h_input_v = self.compute_output_v_for_h(one_hot)
        h_input_y = self.compute_output_y_for_h(labels)

        h_input = [torch.add(h_input_v[i], h_input_y[i]) for i in range(len(h_input_v))]
        h_pos = self.sample_from_inputs_h(h_input)
    ...
    def predict(self, X):
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(data_loader):
                ...
                likelihood_preds += self.likelihood(one_hot, predicted_labels).detach().tolist()

        return X.sequence.tolist(), likelihood_preds, label_preds

    def label_prediction(self, v):
        h_input_v = self.compute_output_v_for_h(v)
        h_input_y = self.compute_output_y_for_h(self.random_init_config_y(custom_size=(v.shape[0],)))
        h_input = [torch.add(h_input_v[i], h_input_y[i]) for i in range(len(h_input_v))]

        h_sampled = self.sample_from_inputs_h(h_input)
        y_input = self.compute_output_h_for_y(h_sampled)
        return self.sample_from_inputs_y(y_input)
    ...
    def random_init_config_y(self, custom_size=False, zeros=False):
        ...
        if zeros:
            ...
            return torch.randint(self.classes+1, size, device=self.device)
    ...
    def gen_data(self, Nchains=10, Lchains=100, Nthermalize=0, Nstep=1, N_PT=1, config_init=[], beta=1, batches=None, reshape=True, record_replica=False, record_acceptance=None, update_betas=None, record_swaps=False):
        ...
        with torch.no_grad():
            ...
            if reshape:
                return [data[0].flatten(0, -3), [hd.flatten(0, -3) for hd in data[1]], data[2].flatten(0, -3)]
            else:
                return data
    ...
    def on_before_zero_grad(self, optimizer):
        ...
        with torch.no_grad():
            for key in self.hidden_convolution_keys:
                getattr(self, f"M_{key}").data.clamp_(0.0, 1.0)
